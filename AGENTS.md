...
- Login chỉ verify bằng bcrypt hash trong bảng `users` — KHÔNG có `.env`/fallback. Tạo/reset mật khẩu admin qua trang `admin/setup.php` (form nhập password + confirm + **Database key**, insert/update hash trong `users` + ghi key): upload file → mở URL → nhập mật khẩu → **xoá file khỏi server**. Khi đã tồn tại tài khoản admin, trang yêu cầu nhập **Database key hiện tại** (bằng chứng sở hữu, chống chiếm quyền nếu quên xóa file); bỏ trống New database key khi reset sẽ giữ nguyên key cũ. `admin/login.php` hiện link nhắc tới `admin/setup.php` khi bảng `users` rỗng.
